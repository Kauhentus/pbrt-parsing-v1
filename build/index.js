...
const linalg_matrix_1 = require("./geo/linalg_matrix");
const linalg_standard_1 = require("./geo/linalg_standard");
const ml_matrix_1 = require("ml-matrix");
const params_to_mdl_1 = require("./params_to_mdl");
const vattrib_to_obj_1 = require("./vattrib_to_obj");
...
const parse_pbrt_file = (path) => {
    ...
    const lines = txt
        ...
        if (comment_index == -1)
            ...
            return line.slice(0, comment_index);
    })
        ...
        if (line.includes('AttributeBegin')) {
            ...
            if (!met_trianglemesh && !met_bilinearmesh) {
                ...
                if (index_uv != -1 && index_N != -1 && index_P != -1 && index_ind != -1) {
                    const list = [[index_uv, 'uv'], [index_N, 'N'], [index_P, 'P'], [index_ind, 'ind'], [Infinity, 'end']];
                    list.sort((a, b) => a[0] - b[0]);
                    const index_range_to_array = (start, end) => {
                    ...
                    // }
                    (0, vattrib_to_obj_1.vattrib_to_obj)(`${output_directory}/geometry_custom/mesh_custom_${file_name_counter}.obj`, data_uv, data_N, data_P, data_ind);
...
const include_handling = (path) => {
    ...
    for (let i of indices_to_replace) {
        const statement = results[i];
        const new_path = statement.params[0];
...
if (!fs.existsSync('./cached_statements.json') || !use_cached_results) {
    ...
    intermediate_statements = JSON.parse(fs.readFileSync('./cached_statements.json').toString());
}
...
`;
let ctm_stack = [];
let ctm = (0, linalg_matrix_1.mat4_identity)();
...
let texture_dictionary = new Map();
let is_light_stack = [];
let is_light = false;
let current_material_stack = [];
...
for (let statement of intermediate_statements) {
    ...
    if (statement.kind === 'Camera') {
        camera.camera[2] = statement.attributes[0].values[0];
    }
    else if (statement.kind === 'LookAt') {
        console.log(JSON.stringify(statement, null, 4));
        const p = statement.params;
        const eye = [p[0], p[1], p[2]];
        const target = [p[3], p[4], p[5]];
        const up_vector = [p[6], p[7], p[8]];
        camera.center = eye;
        const look_vector = (0, linalg_matrix_1.vec3_sub)(target, eye);
        const r = (0, linalg_standard_1.magnitude_vec3)(look_vector);
    ...
    if (statement.kind === 'AttributeStart') {
        ...
        is_light = is_light_stack.pop();
        if (is_light === undefined)
            is_light = false;
        current_material = current_material_stack.pop();
        if (current_material === undefined)
            current_material = 'default';
        ctm = ctm_stack.pop();
        if (ctm === undefined)
            ctm = (0, linalg_matrix_1.mat4_identity)();
    }
    if (statement.kind === "AreaLightSource") {
        is_light = true;
    }
    else if (statement.kind === 'NamedMaterial') {
        let material_name = statement.params[0];
        ...
        if (roughness_index != -1)
            roughness = statement.attributes[roughness_index].values[0];
        let type = 'coateddiffuse';
        if (type_index != -1)
            type = statement.attributes[type_index].values[0];
        let color_texture_name = '';
        if (color_texture_index != -1)
            color_texture_name = statement.attributes[color_texture_index].values[0];
        let normal_map_path = '';
        if (normal_map_index != -1)
            normal_map_path = statement.attributes[normal_map_index].values[0];
        if (copy_normal_maps && normal_map_path.length > 0) {
            fs.copyFileSync(`${root_dir}/${normal_map_path}`, `${output_directory}/${mdl_folder_name}/${normal_map_path}`);
        }
        let rgb_reflectance = [0, 0, 0];
        if (rgb_reflectance_index != -1)
            rgb_reflectance = statement.attributes[rgb_reflectance_index].values;
        let mix_materials = [];
        if (mix_materials_index != -1)
            mix_materials = statement.attributes[mix_materials_index].values;
        let params = {
            material_name: material_name,
        ...
        if (write_mdl_files)
            (0, params_to_mdl_1.params_to_mdl)(params, texture_dictionary, output_directory, mdl_folder_name, material_name == 'Canvas_02');
        final_file_content += `
mdl ${material_name} ${material_name} "${mdl_folder_name}/${material_name}.mdl"
`;
    }
    else if (statement.kind === 'Texture') {
        const texture_name = statement.params[0];
        const path_index = statement.attributes.findIndex(a => a.name === 'string filename');
        let texture_path;
        if (path_index != -1) {
            texture_path = statement.attributes[path_index].values[0];
        }
        else {
            // is mix material
            ...
            texture_path = texture_dictionary.get(statement.attributes[index].values[0]);
        }
        if (copy_texture_files) {
            fs.copyFileSync(`${root_dir}/${texture_path}`, `${output_directory}/${mdl_folder_name}/${texture_path}`);
        }
        texture_dictionary.set(texture_name, texture_path);
    }
    else if (statement.kind === 'Transform') {
        ctm = statement.params;
        translation = [ctm[12], ctm[13], ctm[14]];
        scale = [
            (0, linalg_standard_1.magnitude_vec3)([ctm[0], ctm[1], ctm[2]]),
            (0, linalg_standard_1.magnitude_vec3)([ctm[4], ctm[5], ctm[6]]),
            (0, linalg_standard_1.magnitude_vec3)([ctm[8], ctm[9], ctm[10]]),
        ];
        const R = new ml_matrix_1.Matrix([
        ...
        const e = new ml_matrix_1.EigenvalueDecomposition(R);
        ...
        const p = statement.params;
        const new_mat = (0, linalg_matrix_1.mat4_translate)(p[0], p[1], p[2]);
        translation = [p[0], p[1], p[2]];
        ctm = (0, linalg_matrix_1.mat4_matmul)(new_mat, ctm);
    }
    else if (statement.kind === 'Rotate') {
        const p = statement.params;
        const new_mat = (0, linalg_matrix_1.mat4_rot_axis)([p[1], p[2], p[3]], p[0]);
        rotation = [p[1], p[2], p[3], p[0]];
        ctm = (0, linalg_matrix_1.mat4_matmul)(new_mat, ctm);
    }
    else if (statement.kind === 'Scale') {
        const p = statement.params;
        const new_mat = (0, linalg_matrix_1.mat4_scale)(p[0], p[1], p[2]);
        ctm = (0, linalg_matrix_1.mat4_matmul)(new_mat, ctm);
        scale = [p[0], p[1], p[2]];
    }
        ...
        const model_path = statement.attributes[0].values[0];
        if (bad_files.some(file => model_path.includes(file))) {
            continue;
        }
        if (copy_model_files && !model_path.includes("mesh_custom")) {
            fs.copyFileSync(`${root_dir}/${model_path}`, `${output_directory}/${model_path}`);
        }
        final_file_content += `
push 
